...
public partial class mecommerce {
    ...
    /// subscriptionsList
    /// </summary>
    public static SubscriptionsList subscriptionsList
    {
        get => HttpData.Get<SubscriptionsList>("subscriptionsList")!;
        set => HttpData["subscriptionsList"] = value;
    }

    /// <summary>
    /// Page class for Subscriptions
    /// </summary>
    public class SubscriptionsList : SubscriptionsListBase
    {
        // Constructor
        public SubscriptionsList(Controller controller) : base(controller)
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class SubscriptionsListBase : Subscriptions
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "Subscriptions";

        // Page object name
        public string PageObjName = "subscriptionsList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fSubscriptionslist";
        ...
        // Constructor
        public SubscriptionsListBase()
        {
            // CSS class name as context
            TableVar = "Subscriptions";
            ...
            if (subscriptions == null || subscriptions is Subscriptions)
                subscriptions = this;

            // Initialize URLs
            AddUrl = "subscriptionsadd";
            ...
            MultiDeleteUrl = "subscriptionsdelete";
            MultiUpdateUrl = "subscriptionsupdate";
        ...
        // Page name
        public string PageName => "subscriptionslist";
        ...
        public void SetVisibility()
        {
            ...
            PublicKey.SetVisibility();
            AuthenticationToken.SetVisibility();
            ContentEncoding.SetVisibility();
        }

        // Constructor
        public SubscriptionsListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "subscriptionsview" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fSubscriptionssrch");
            ...
            filters.Merge(JObject.Parse(Endpoint.AdvancedSearch.ToJson())); // Field Endpoint
            filters.Merge(JObject.Parse(PublicKey.AdvancedSearch.ToJson())); // Field PublicKey
            filters.Merge(JObject.Parse(AuthenticationToken.AdvancedSearch.ToJson())); // Field AuthenticationToken
            filters.Merge(JObject.Parse(ContentEncoding.AdvancedSearch.ToJson())); // Field ContentEncoding
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fSubscriptionssrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Endpoint
            if (filter?.TryGetValue("x_Endpoint", out sv) ?? false) {
                Endpoint.AdvancedSearch.SearchValue = sv;
                Endpoint.AdvancedSearch.SearchOperator = filter["z_Endpoint"];
                Endpoint.AdvancedSearch.SearchCondition = filter["v_Endpoint"];
                Endpoint.AdvancedSearch.SearchValue2 = filter["y_Endpoint"];
                Endpoint.AdvancedSearch.SearchOperator2 = filter["w_Endpoint"];
                Endpoint.AdvancedSearch.Save();
            }

            // Field PublicKey
            if (filter?.TryGetValue("x_PublicKey", out sv) ?? false) {
                PublicKey.AdvancedSearch.SearchValue = sv;
                PublicKey.AdvancedSearch.SearchOperator = filter["z_PublicKey"];
                PublicKey.AdvancedSearch.SearchCondition = filter["v_PublicKey"];
                PublicKey.AdvancedSearch.SearchValue2 = filter["y_PublicKey"];
                PublicKey.AdvancedSearch.SearchOperator2 = filter["w_PublicKey"];
                PublicKey.AdvancedSearch.Save();
            }

            // Field AuthenticationToken
            if (filter?.TryGetValue("x_AuthenticationToken", out sv) ?? false) {
                AuthenticationToken.AdvancedSearch.SearchValue = sv;
                AuthenticationToken.AdvancedSearch.SearchOperator = filter["z_AuthenticationToken"];
                AuthenticationToken.AdvancedSearch.SearchCondition = filter["v_AuthenticationToken"];
                AuthenticationToken.AdvancedSearch.SearchValue2 = filter["y_AuthenticationToken"];
                AuthenticationToken.AdvancedSearch.SearchOperator2 = filter["w_AuthenticationToken"];
                AuthenticationToken.AdvancedSearch.Save();
            }

            // Field ContentEncoding
            if (filter?.TryGetValue("x_ContentEncoding", out sv) ?? false) {
                ContentEncoding.AdvancedSearch.SearchValue = sv;
                ContentEncoding.AdvancedSearch.SearchOperator = filter["z_ContentEncoding"];
                ContentEncoding.AdvancedSearch.SearchCondition = filter["v_ContentEncoding"];
                ContentEncoding.AdvancedSearch.SearchValue2 = filter["y_ContentEncoding"];
                ContentEncoding.AdvancedSearch.SearchOperator2 = filter["w_ContentEncoding"];
                ContentEncoding.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Endpoint);
            searchFlds.Add(PublicKey);
            searchFlds.Add(AuthenticationToken);
            searchFlds.Add(ContentEncoding);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(PublicKey); // PublicKey
                UpdateSort(AuthenticationToken); // AuthenticationToken
                UpdateSort(ContentEncoding); // ContentEncoding
        ...
        protected void ResetCommand() {
            ...
            if (Command.ToLower().StartsWith("reset")) {
                ...
                if (SameText(Command, "resetsort")) {
                    ...
                    Endpoint.Sort = "";
                    PublicKey.Sort = "";
                    AuthenticationToken.Sort = "";
                    ContentEncoding.Sort = "";
                }
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""Subscriptions"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""Subscriptions"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""Subscriptions"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubscriptionslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fSubscriptionslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""Subscriptions"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fSubscriptionslist""" +
            ...
            item.Body = GetPushNotificationTag();
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("PublicKey")); // DN
                CreateColumnOption(option.Add("AuthenticationToken")); // DN
                CreateColumnOption(option.Add("ContentEncoding")); // DN
            }
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fSubscriptionslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_Subscriptions");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(subscriptionsList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(subscriptionsList.RowCount) + "_Subscriptions");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(subscriptionsList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && subscriptionsList.RowType == RowType.Add || IsEdit && subscriptionsList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            Endpoint.SetDbValue(row["Endpoint"]);
            PublicKey.SetDbValue(row["PublicKey"]);
            AuthenticationToken.SetDbValue(row["AuthenticationToken"]);
            ContentEncoding.SetDbValue(row["ContentEncoding"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Endpoint", Endpoint.DefaultValue ?? DbNullValue); // DN
            row.Add("PublicKey", PublicKey.DefaultValue ?? DbNullValue); // DN
            row.Add("AuthenticationToken", AuthenticationToken.DefaultValue ?? DbNullValue); // DN
            row.Add("ContentEncoding", ContentEncoding.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // PublicKey
                PublicKey.ViewValue = ConvertToString(PublicKey.CurrentValue); // DN
                PublicKey.ViewCustomAttributes = "";

                // AuthenticationToken
                AuthenticationToken.ViewValue = ConvertToString(AuthenticationToken.CurrentValue); // DN
                AuthenticationToken.ViewCustomAttributes = "";
                ...
                // PublicKey
                PublicKey.HrefValue = "";
                PublicKey.TooltipValue = "";

                // AuthenticationToken
                AuthenticationToken.HrefValue = "";
                AuthenticationToken.TooltipValue = "";

                // ContentEncoding
                ContentEncoding.HrefValue = "";
                ContentEncoding.TooltipValue = "";
            }
