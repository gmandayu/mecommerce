...
public partial class mecommerce {
    ...
    public class Index : IndexBase
    {
        ...
        public override void PageRedirecting(ref string url) {
            //url = newurl;
            string refUrl = HttpContext.Request.Headers["Referer"].ToString();
            string userLevel = CurrentUserLevel().ToString();
            if (userLevel == "-2") { url = "login"; }
            if (userLevel == "-1") { url = "userslist"; }
            if (userLevel == "0") {
                var rs = ExecuteRow($"SELECT TOP 1 CustomerID, Status " +
                    $"FROM Customers " +
                    $"WHERE UserID = {CurrentUserID().ToString()}");
                if (rs == null)
                {
                    rs = ExecuteRow($"SELECT * FROM Users " +
                        $"WHERE id = {CurrentUserID()}");
                    QueryBuilder("Customers").Insert(new
                    {
                        MobileNumber = rs["MobileNumber"] != null ? rs["MobileNumber"].ToString() : "",
                        Status = "Draft",
                        UserID = Convert.ToInt32(rs["UserID"]),
                        CreatedBy = Convert.ToInt32(rs["UserID"]),
                        CreatedDateTime = DateTimeOffset.Now,
                        UpdatedBy = Convert.ToInt32(rs["UserID"]),
                        UpdatedDateTime = DateTimeOffset.Now,
                    });
                }
                dynamic customerQueryResult = QueryBuilder("Customers")
                    .Where("UserID", CurrentUserID())
                    .Select("CustomerID").FirstOrDefault();
                if ((object)customerQueryResult != null)
                {
                    int customerID = customerQueryResult.id;
                    url = $"customersedit/{customerID.ToString()}";
                }
            }
            if (userLevel == "1") { url = "subscriptionslist"; }
            if (userLevel == "2") { url = "subscriptionslist"; }
        }

        private void CustomFunctionTest(string p) {
            Console.WriteLine(p);
        }
    }
    ...
    public class IndexBase : AspNetMakerPage
    {
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            for (var i = 0; i < tableList.Count; i++) {
                if (tableList[i].TableName == "UserLevelPermissions") {
