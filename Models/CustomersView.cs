...
public partial class mecommerce {
    ...
    {
        get => HttpData.Get<CustomersView>("customersView")!;
        set => HttpData["customersView"] = value;
    }

    /// <summary>
    /// Page class for Customers
    /// </summary>
    public class CustomersView : CustomersViewBase
    {
        // Constructor
        public CustomersView(Controller controller) : base(controller)
        {
        }

        // Constructor
        public CustomersView() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class CustomersViewBase : Customers
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "customersView";
        ...
        public CustomersViewBase()
        {
            ...
            if (IsApi()) {
                ...
                if (keys.Length > 0)
                    RecordKeys["CustomerID"] = keys[0];
            } else {
                RecordKeys["CustomerID"] = RouteValues.TryGetValue("CustomerID", out obj) && obj != null ? UrlDecode(obj) : Get("CustomerID"); // DN
            }
        ...
        // Page name
        public string PageName => "customersview";
        ...
        // Constructor
        public CustomersViewBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (RouteValues.TryGetValue("CustomerID", out v) && !Empty(v)) { // DN
                CustomerID.QueryValue = UrlDecode(v); // DN
                RecordKeys["CustomerID"] = CustomerID.QueryValue;
            } else if (Get("CustomerID", out sv)) {
                CustomerID.QueryValue = sv.ToString();
                RecordKeys["CustomerID"] = CustomerID.QueryValue;
            } else if (IsApi() && !Empty(keyValues)) {
                CustomerID.QueryValue = ConvertToString(keyValues[0]);
                RecordKeys["CustomerID"] = CustomerID.QueryValue;
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                ...
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-email\" title=\"" + Language.Phrase("ExportToEmail", true) + "\" data-caption=\"" + Language.Phrase("ExportToEmail", true) + "\" form=\"fCustomersview\" data-ew-action=\"email\" data-hdr=\"" + Language.Phrase("ExportToEmail", true) + "\" data-key='" + HtmlEncode(ConvertToJsonAttribute(RecordKeys)) + "' data-export-selected=\"false\"" + url + ">" + Language.Phrase("ExportToEmail") + "</button>";
